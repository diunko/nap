...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  onNapkinStatusChanged: (callback: (data: { slug: string; status: string }) => void) => {
    const handler = (_event: IpcRendererEvent, data: { slug: string; status: string }) =>
      callback(data);
  ...
  onArchitectUpdate: (callback: (data: unknown) => void) => {
    const handler = (_event: IpcRendererEvent, data: unknown) => callback(data);
    ipcRenderer.on('architect:update', handler);
    return () => ipcRenderer.removeListener('architect:update', handler);
  },
  ...
  getInitialTerminalOpts: () => ipcRenderer.invoke('get-initial-terminal-opts') as Promise<{ name: string; command?: string; architect?: boolean }>,
