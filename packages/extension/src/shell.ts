/**
 * BashShell — copied from bash-poc/src/shell.ts.
 * Identical logic, separate copy for extension package.
 */
...
function defaultPrompt(cwd: string): string {
  ...
  return `\x1b[1;32muser@nap\x1b[0m:\x1b[1;34m${display}\x1b[0m$ `;
}
...
export class BashShell {
  ...
  async handleInput(data: string): Promise<void> {
    ...
    if (data === '\r') {
      ...
      if (this._cursor > 0) { this._cursor--; write('\x1b[D'); }
    } else if (data === '\x1b[C') {
      if (this._cursor < this._line.length) { this._cursor++; write('\x1b[C'); }
    } else if (data === '\x15') {
      ...
      if (this._cursor > 0) { write(`\x1b[${this._cursor}D`); this._cursor = 0; }
    } else if (data === '\x05') {
  ...
  private async _tabComplete(): Promise<void> {
    ...
    try {
      const result = await bash.exec(`ls -1a ${JSON.stringify(dir)}`, { cwd: this._cwd });
    ...
    if (isFirst && !word.includes('/')) {
      ...
      } catch { /* compgen may not be available */ }
    }
