 ...
 *
 * Maps lightning-fs `.promises` API to the just-bash IFileSystem interface.
...
export class LightningFsAdapter implements IFileSystem {
  ...
  async chmod(_path: string, _mode: number): Promise<void> {
    // lightning-fs doesn't support chmod — no-op
  }

  async utimes(_path: string, _atime: Date, _mtime: Date): Promise<void> {
    // lightning-fs doesn't support utimes — no-op
  }
  ...
  async link(_existingPath: string, _newPath: string): Promise<void> {
    // lightning-fs doesn't support hard links — throw
  ...
  async realpath(path: string): Promise<string> {
    // Walk the path resolving symlinks at each component
    const parts = normalizePath(path).split('/').filter(Boolean);
    let resolved = '';
    for (const part of parts) {
      resolved += '/' + part;
      try {
        ...
        // path component doesn't exist — just keep going
      }
    }
    return resolved;
  }
