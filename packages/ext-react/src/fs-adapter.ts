...
// ── Change event emitter (the extension's fs.watch) ──
...
export class LightningFsAdapter implements IFileSystem {
  ...
  private changeListeners: FsChangeListener[] = [];
  ...
  private emit(event: FsChangeEvent): void {
    console.log(`[adapter] emit ${JSON.stringify(event)}`);
  ...
  async writeFile(path: string, content: FileContent, _options?: { encoding?: BufferEncoding } | BufferEncoding): Promise<void> {
    console.log(`[adapter] writeFile ${path}`);
