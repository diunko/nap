...
* J1: the spark
  ...
  * first-time setup
    ...
    * do: run `nap open --architect`
      * see: Electron app launches — three-column layout appears. Gutter shows one nepic icon ("V") with white active bar. Sidebar shows "001-architect" with green pulsing dot. Terminal shows architect booting with `claude --verbose --session-id <uuid>`.
...
* J4: the nap
  ...
  * closing and reopening
    ...
    * do: reopen the app (or run `nap open`)
      * see: app restores to previous state. Architect auto-resumes via `claude --resume <uuid>`. Other claude sessions show as orphaned dots (dashed border, dimmed). All napkin cards, phase badges, and sidebar layout restored.
      * so that: you pick up exactly where you left off — the tool remembers everything

    * do: click an orphaned agent dot in sidebar
      * see: agent resumes — orphaned dot becomes solid green (running). Terminal shows Claude picking up mid-conversation. Breadcrumb shows agent path.
      * so that: any agent can be brought back with one click — they have full context from before
...
* cross-cutting interactions
  ...
  * waiting for agents
    ...
    * do: architect runs `nap nap TE-100` (waits for agent to finish)
      * see: command blocks until TE-100 calls `nap done`, then prints the done message and exits
